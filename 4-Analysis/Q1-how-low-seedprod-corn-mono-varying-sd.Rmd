---
title: 'Q1: how low is seed production allowed: corn monoculture, continued'
output: html_document 
---
## Follow-up from "4-Analysis/Q1-how-low-seed-prod-corn-mono-varying-mean", try varying sd, set ln_mean between 6 and 6.7 
...
#### at t = 1: 1000 seeds, from t = 2: rlnorm(1, 6.5, 0.86)
...
corn1 <- function(vec, poh, ow, prt, em, sv){
   ...
   mature_plant_total <-  early_summer_sb[1]*em[3,1]*sv[[3,3]] + 
    ...
    new_seeds_total<-  mature_plant_total * rlnorm(1, 6.5, 0.86)
...
#### at t = 1: 1000 seeds, from t = 2: rlnorm(1, 6.7, 0.95)
...
corn2 <- function(vec, poh, ow, prt, em, sv){
   ...
   mature_plant_total <-  early_summer_sb[1]*em[3,1]*sv[[3,3]] + 
    ...
    new_seeds_total <-  mature_plant_total * rlnorm(1, 6.5, 0.95) #whole-population annual fecundity
...
#### at t = 1: 1000 seeds, from t = 2: rlnorm(1, 6.5, 0.6)
...
corn3 <- function(vec, poh, ow, prt, em, sv){
   ...
   mature_plant_total <-  early_summer_sb[1]*em[3,1]*sv[[3,3]] + 
    ...
    new_seeds_total <-  mature_plant_total *  rlnorm(1, 6.5, 1.1)
...
#### at t = 1: 1000 seeds, from t = 2: rlnorm(1, 6.5, 0.4)
...
corn4 <- function(vec, poh, ow, prt, em, sv){
   ...
   mature_plant_total <-  early_summer_sb[1]*em[3,1]*sv[[3,3]] + 
    ...
    new_seeds_total <-  mature_plant_total *  rlnorm(1, 6.5, 0.4)
...
Q <- list() # blank dataframe to save loop output 
  ## Calculate P0
Q0 <- corn3(vec = starting_point[2:3], 
   poh = chisel, 
   ow = overwinter,
   prt = field_cultivator,
   em = emergence[["C2_conv"]],
...
#### at t = 1: 1000 seeds, from t = 2: rlnorm(1, 6.5, 0.25)
...
corn4 <- function(vec, poh, ow, prt, em, sv){
   ...
   mature_plant_total <-  early_summer_sb[1]*em[3,1]*sv[[3,3]] + 
    ...
    new_seeds_total <-  mature_plant_total *  rlnorm(1, 6.5, 0.25)
...
R <- list() # blank dataframe to save loop output 
  ## Calculate P0
R0 <- corn3(vec = starting_point[2:3], 
   poh = chisel, 
   ow = overwinter,
   prt = field_cultivator,
   em = emergence[["C2_conv"]],
