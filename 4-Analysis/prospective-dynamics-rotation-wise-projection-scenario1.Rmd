...
Variance of annualized population growth rate is calculated with Taylor series expansion using the general formula: $Var[f(\lambda)] = {f'(E(\lambda))}^2 Var(\lambda)$, in which $f(\lambda)$ is the nth-root function used to annualize rotation-wise growth rate. For example, the variance of annualized population growth rate in the 2-year rotation was $Var(\lambda_{2}) = \frac{1}{4\lambda_{2r}}Var(\lambda_{2r})$, where $\lambda_2 = \sqrt{\lambda_{2r}}$.  
...
var_lambda_table <- expand.grid(Corn_weed_management = c("conventional", "low"),
                                Rotation = c("2-year", "3-year", "4-year"))
# combine all variance into a data frame and calculate annualized lambda's var

var_lambda_table$Rotation_lambda <- c(var_lambda_conv_2yr,
                                      var_lambda_low_2yr,
                                      var_lambda_conv_3yr,
                                      var_lambda_low_3yr,
                                      var_lambda_conv_4yr,
...
var_lambda_table$multiplier <- c(1/4, 1/4, 1/9, 1/9, 1/16, 1/16)


# combine lambda, var(lamda), and elasticity contribution and rename the data frame
scenario1_annualized_lambda_and_var <- scenario1_annualized_lambda_df_long %>%
  mutate(squared_exp_annualized_lambda = annualized_lambda^2,
         var_annualized_lambda = multiplier * var_lambda_rot/squared_exp_annualized_lambda)

# check: var(annualized lambda) is very large, but the var component of fecundity is 0. 


# add E_B contributions to lambda's elasticity 
```
