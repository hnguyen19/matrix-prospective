...
contribution_4yr <- purrr::map2(D4_avg_contribution_transposed, A4_avg_sens, `%*%`)
# remove "conv" in all names
contribution_4yr_raw_names <- names( contribution_4yr )

contribution_4yr_names <- gsub('conv.', '', contribution_4yr_raw_names)

names(contribution_4yr) <- contribution_4yr_names
```

```{r elasticity-4yr}
## calculate elasticity, see p. 121

# Elasticities with regards to the right-most matrix --> 
A4_conv_elas <- A4_conv_contribution %>%
  map(., ~{elasticity(.)})

A4_low_elas <- A4_low_contribution %>%
  map(., ~{elasticity(.)})
