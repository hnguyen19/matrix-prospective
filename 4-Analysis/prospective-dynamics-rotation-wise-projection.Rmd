...
# The model clock starts at seed production (so this sub-annual matrix is listed right-most)
#

scenario1_projection_by_matrix_id <- pmap(list(post_harvest_tillage,
...
scenario1_rot_projection <- scenario1_list_rotation %>%
  map(., ~{Reduce( "%*%", .)})

# eigen analysis
scenario1_rotation_eigen <- scenario1_rot_projection  %>%
  map(., ~{eigen.analysis(.)})
```
...
scenario2_projection_by_matrix_id <- pmap(list(post_harvest_tillage,
                                               overwinter_scenario1,
                                               spring_tillage,
                                               emergence_scenario1,
                                               summer_survival_scenario1,
