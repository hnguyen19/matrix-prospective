---
title: 'Q1: how low is seed production plant density - rotation'
...
# soybean phase dynamics
   ...
   density_in_corn <- em_C %*% prt_C %*%  ow_C %*%  poh_C %*% vec

   after_soy <- seed_S %*% sv_S %*% em_S %*% prt_S %*%  ow_S %*%  poh_S %*% after_corn 
   
   density_in_soy <-  em_S %*% prt_S %*%  ow_S %*%  poh_S %*% after_corn
...
```{r seed-allowance-sim-plot, echo=FALSE, fig.cap= "Population growth rates over 100 rotational cycles. All simulations started with a seed column of 10000 female seeds in the top 0 - 2 cm soil stratum and 0 female seed in the bottom 2 - 18 cm soil stratum. The simulation applied weed management on cohorts 1 through 3 in corn and soybean only. The relationships of aboveground mass and fecundity in Nguyen and Liebman (2022b) were used to estimate cohort-based fecundity. In corn and soybean, the fecundity estimated through aboveground mass of cohorts 1 through 3 fecundity were manipulated to find the seed allowance in the corn and soybean environments, but the fecundity estimated through aboveground mass of cohorts 4 and beyond were kept as they were. Each panel was annotated with the average fecundity of cohorts 1 through 3."}
...
seed_allowance_sim_df %>%
 ...
 # geom_line(aes(y = lambda_annualized, color =  category)) +
  geom_point(aes(y  = lambda_annualized, color =  category)) +
...
#  geom_hline(yintercept = 1, color = "red") +
  ...
  theme(legend.position = "bottom",
    ...
    geom_text(data = sim_label, aes(label = label), size = 3, x = 50, y = 1.5) 
```