...
scenario1_projection_by_matrix_id <- pmap(list(fecundity_2019,
                                               post_harvest_tillage,
                                               overwinter_scenario1), list) %>% 
  simplify() %>%
  map(.,
    ~{names(.) <-  c("fecundity_2019",
                     "post_harvest_tillage",
                     "overwinter_Sosnoskie_2013"); .}) #wrong

# multiply all the periodic matrices within a crop ID
scenario1_phase_projection <- scenario1_projection_by_matrix_id  %>%
...
scenario1_rotation_projection <- scenario1_list_rotation %>%
  map(.,~{Reduce("%*%", .)}) 
```
...
scenario2_projection_by_matrix_id <- pmap(list(fecundity_2018,
                                               post_harvest_tillage,
                                               overwinter_scenario1), list) %>% 
  simplify() %>%
  map(.,
    ~{names(.) <-  c("fecundity_2018",
                     "post_harvest_tillage",
                     "overwinter_Sosnoskie_2013"); .}) 

# multiply all the periodic matrices within a crop ID
scenario2_phase_projection <- scenario2_projection_by_matrix_id  %>%
  map(., ~{Reduce( "%*%", .)}) #not crossprod, because crossprod(X,Y) = t(X)%*%Y

```

```{r scenario-2-4yr-only}
...
scenario2_list_rotation <- list(scenario2_rotation_wise_4yr_conv,
                                     scenario2_rotation_wise_4yr_low) 
names(scenario2_list_rotation) <- c("4_year_conv",
                                    "4_year_low")

# Rotation-wise projection: 

scenario2_rotation_projection <- scenario2_list_rotation %>%
  map(.,~{Reduce("%*%", .)})  #makes everything the same
