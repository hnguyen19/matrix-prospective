...
```{r rot-2yr-function-seebank-output, echo=TRUE}
...
N_2yr_conv_df <-  N_2yr_conv %>% 
  ...
  group_by(stratum) %>%
  mutate(lambda_cycle = seedbank_counts/lag(seedbank_counts),
         lambda_annualized = sqrt(lambda_cycle),
          Rotation = "2-year",
         Corn_weed_management = "conventional") #%>% na.omit() to keep the starting population
```

```{r 2yr-conv-seed-production-per-capita}
### Output: seed production, after manipulation by cohort x crop phase x iteration
...
```{r 2yr-conv-plant-density-fixed}
...
N_2yr_conv_all_df <- N_2yr_conv_seed_production_per_capita_df %>% 
  ...
  mutate(seed_production = seed_per_capita*plant_counts)


# View(N_2yr_conv_all_df )
```
