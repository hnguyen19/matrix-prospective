...
## convert log-scaled numbers back to original 
seeds_18_emm_original <- summary(seeds_18_emm, type = "response")

seeds_18_emm_df <- seeds_18_emm_original  %>%
  as.data.frame() %>%
  rename(Seed_mean = response,
         Seed_SE = SE) %>%
  mutate(Seed_var = Seed_SE^2)
