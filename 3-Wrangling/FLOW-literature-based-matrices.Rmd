...
Matrices in this document is listed chronologically, from natural seed rain (at AMATA senescence) to new seed production. 
...
- Sum the first three elements of the bottom-up vector to have $T_{2,1}$ and the last sixteem elements to have $T_{2,2}$.
...
```{r off-season-viability-matrix, echo=FALSE}
...
# Assign 6 months of summer and winter to corn and soybean, 5.5 months of summer to oat and 4.5 month of summer to alfalfa per the observation at Marsden that oat and alfalfa delayed waterhemp emergence.
...
overwinter_Sosnoskie_13_cohort_list <- lapply(split(overwinter_Sosnoskie_13_cohort, overwinter_Sosnoskie_13_cohort$matrix_id),
       function(x)(rbind(cbind(diag(x$Seed_overwinter_prop, 2), 
                         matrix(rep(0,12), nrow = 2)),
                   matrix(rep(0,48), nrow = 6))))
                         
off_season_scenario1 <- overwinter_Sosnoskie_13_cohort_list 

# saveRDS(off_season_scenario1 , file="../2-Data/Clean/winter-seed-survival-scenario1.RData")
...
pre_planting_tillage <- rbindlist(list(field_cultivator, field_cultivator , field_cultivator, field_cultivator,
...
pre_planting_tillage$matrix_id <- c(rep("C2_low", 2), rep("C2_conv",2),
                                    rep("C3_low", 2), rep("C3_conv",2),
                                    rep("C4_low", 2), rep("C4_conv",2),
                                    rep("S2_low", 2), rep("S2_conv",2),
                                    rep("S3_low", 2), rep("S3_conv",2),
...
pre_planting_tillage_list <- lapply(split(pre_planting_tillage, pre_planting_tillage$matrix_id),
      function(x)(rbind(cbind(matrix(c(x$X1, x$X2), byrow = FALSE, nrow=2),
                   matrix(rep(0,12), nrow = 2)),
                   matrix(rep(0,48), nrow = 6))))

#saveRDS(pre_planting_tillage_list, file="../2-Data/Clean/pre-planting-tillage.RData")
```
...
summer_seed_survival_Sosnoskie_13_cohort <- summer_seed_survival_Sosnoskie_13  %>%
  mutate(Seed_overwinter = Coefficient_a*exp(Coefficient_b*summer_month),
         Seed_overwinter_prop = Seed_overwinter/100,
         matrix_id = paste(from_Phase,Corn_weed_management, sep = "_")) %>%
  arrange(matrix_id, desc(Stratum)) #Matrices are identified by the crop species that presented before the month of December in a particular plot.
...
summer_seed_survival_Sosnoskie_13_cohort_list  <- lapply(split(summer_seed_survival_Sosnoskie_13_cohort , summer_seed_survival_Sosnoskie_13_cohort$matrix_id),
       function(x) rbind(cbind(diag(x$Seed_overwinter_prop, 2), 
                         matrix(rep(0,12), nrow = 2)), cbind(matrix(0,nrow = 6, ncol = 2), diag(1,6))))


# saveRDS(summer_seed_survival_Sosnoskie_13_cohort_list, file="../2-Data/Clean/summer-seed-survival-scenario1.RData")
...
## Fill in the proportion of retrieved seeds and viability of retrieved seeds 1 year after burial 
...
summer_seed_survival_scenario2_filled_list <- lapply(split(summer_seed_survival_scenario2_filled, summer_seed_survival_scenario2_filled$matrix_id),
       function(x) rbind(cbind(diag(x$original_survival_prop, 2), 
                         matrix(rep(0,12), nrow = 2)), cbind(matrix(0,nrow = 6, ncol = 2), diag(1,6))))

# saveRDS(summer_seed_survival_scenario2_filled_list, file="../2-Data/Clean/summer-seed-survival-scenario2.RData")
```
