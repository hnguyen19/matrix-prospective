...
var_post_harvest_tillage_list <- lapply(split(var_post_harvest_tillage_df, var_post_harvest_tillage_df$matrix_id),
       function(x) matrix(c(x$phV1, x$phV2), byrow = FALSE, nrow=2)
...
The field cultivator mean and variance matrices were calculated using the same procedure as for chisel and moldboard ploughs.

```{r pre-planting-tillage-matrix-new}
...
var_pre_planting_tillage_list <- lapply(split(var_pre_planting_tillage_df, var_pre_planting_tillage_df$matrix_id),
      function(x) matrix(c(x$V1, x$V2), byrow = FALSE, nrow=2))
...
seedling_survival_soybean_filled <- seedling_survival_soybean %>%
  ...
  mutate(const. = 95.4, # the constant variable in the y = a + bx + cx^2
         ...
         number_of_plant = 60,
         var_seedling_survival = mean_seedling_survival*(1-mean_seedling_survival)) 
...
seedling_survival_soybean_filled <- seedling_survival_soybean_filled %>%
  mutate(var_seedling_survival = ifelse(mean_seedling_survival > 0, mean_seedling_survival*(1-mean_seedling_survival), 0))
