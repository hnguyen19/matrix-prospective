...
The number of seedlings in each cohort was converted to densities (seedlings/m$^2$). The dates of first emergence detection in each crop identity were noted. 
...
$S_{A(h)}$ is the sensitivity of $\lambda_{rw}$ to each element of $A^*_h$ (the average annual projection matrix between the reference treatment and the treatment of interest). In this study, the reference treatment was the conventional corn weed management and the treatment of interest was the low herbicide corn weed management regime. $S_A(h)$ was calculated by applying the `sensitivity` function from the `popbio` package on the $A^*_h$.  
